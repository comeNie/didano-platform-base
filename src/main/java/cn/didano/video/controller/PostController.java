...
public class PostController {
	...
	public Out<OutList<Tb_student_inf>> student_search(@PathVariable("student_id") Integer student_id) {
		...
		try {
			...
			if (studentall.size() > 4) {
				...
				student.add(inf);
				tb = studentall
						.get((int) (Math.random() * (studentall.size() - 1) / 5) + ((studentall.size() - 1) * 3) / 5);
				inf = new Tb_student_inf(tb.getHeight() / 10, tb.getWeight() / 1000, tb.getOrgImgUrl(),
						sdf.format(tb.getCreated()));
				student.add(inf);
				tb = studentall.get(studentall.size() - 1);
				student.add(new Tb_student_inf(tb.getHeight() / 10, tb.getWeight() / 1000, tb.getOrgImgUrl(),
						sdf.format(tb.getCreated())));
			} else if (studentall.size() == 1) {
				for (int i = 0; i < 4; i++) {
					student.add(
							new Tb_student_inf(studentall.get(0).getHeight() / 10, studentall.get(0).getWeight() / 1000,
									studentall.get(0).getOrgImgUrl(), sdf.format(studentall.get(0).getCreated())));
				}
	 ...
	 * @return
	 */
	@ApiOperation(value = "新增视频频道", notes = "新增视频频道，同时在OSS" + "上创建相关资源，包括推流地址和播放地址")
...
//			} 
	...
	@ResponseBody
	public Out<String> channel_edit(
			@ApiParam(value = "编辑频道对象", required = true) @RequestBody In_Channel_Edit c_channel) {
	...
	@ResponseBody
	public Out<String> channel_set_status(
			@ApiParam(value = "频道状态对象", required = true) @RequestBody ChannelStatus channelStatus) {
	...
	public Out<String> channel_remove(@PathVariable("remove_id") int remove_id) {
		...
		try {
			...
			if (rowNum > 0) {
				...
				back.setBackTypeWithLog(BackType.FAIL_DELETE_NO_DELETE,
						"id=" + vd_channel.getId() + ":rowNum=" + rowNum);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	@ResponseBody
	public Out<OutList<Tb_school>> schools_search(
			@ApiParam(value = "学校查询条件，学校名称", required = false) @RequestBody In_School_Search iss) {
	...
	@ResponseBody
	public Out<PageInfo<View_switch_s_c>> auth_switch_get_all(@PathVariable("page") int page,
	...
	@ResponseBody
	public Out<PageInfo<View_switch_s_c>> auth_switch_search(@PathVariable("page") int page,
			@PathVariable("size") int size,
	...
	@ResponseBody
	public Out<String> auth_switch_edit4video(
			@ApiParam(value = "设置付费对象", required = true) @RequestBody In_Switch_Edit4Video swi) {
	...
	public Out<InfoChannel_C> channel_all() {
		...
		try {
			...
			for (Iterator<View_channel_info> iterator = list_all_status.iterator(); iterator.hasNext();) {
				...
				// private int school_id;String school_name;int
				// on_monitor_num;int on_live_num;
				// private int on_video_num;int all_monitor_num;int
				// all_live_num;int all_video_num;
			...
			for (Iterator<InfoChannel> iterator = infoChannelMap.values().iterator(); iterator.hasNext();) {
				...
				if (tmpS != null) {
					...
					if (tmpS.getStatus() == StatusType.IS_USE.getIndex()
							&& tmpS.getDeleted() == DeletedType.N0_DELETED.getValue()) {
						ss.add(info);
					}
				}
			}
	...
	@ResponseBody
	public Out<OutList<Out_Channel_Time_Control>> time_controls_search_by_schoolid(
		...
		try {
			...
			OutList<Out_Channel_Time_Control> outList = new OutList<Out_Channel_Time_Control>(channel_Controls.size(),
					channel_Controls);
			back.setBackTypeWithLog(outList, BackType.SUCCESS_SEARCH_NORMAL);
		} catch (ServiceException e) {
			logger.warn(e.getMessage());
	...
	@ResponseBody
	public Out<OutList<Vd_auth_time_control>> time_controls_search_by_channelid(
			@PathVariable("channel_id") int channel_Id) {
	...
	@ResponseBody
	public Out<String> time_control_add(
			@ApiParam(value = "新增时间控制", required = true) @RequestBody In_Control_Add control_Add) {
	...
	@ResponseBody
	public Out<String> time_control_edit(
			@ApiParam(value = "编辑时间控制", required = true) @RequestBody In_Control_Edit control_Edit) {
