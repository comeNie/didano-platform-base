 ...
 *****************************************************************************/
//https://searchcode.com/codesearch/raw/3305842/
...
public class TimeUtil {
	 ...
	 *            month 1..12
	 * @param year
	 *            year (if two diguts: < 50 is 2000; > 50 is 1900)
	 ...
	 * @return
	 */
	static public boolean inRange(String start, String end) {
		SimpleDateFormat formatter = new SimpleDateFormat("HH:mm");
		boolean back = false;
		try {
			String startTime = formatter.format(formatter.parse(start)); // 格式化时间为
			String endTime = formatter.format(formatter.parse(end)); // 格式化时间为
			// 创建当前时间currnetTime
			String currentTime = formatter.format(new Date());
			if (currentTime.compareTo(startTime) > 0 && currentTime.compareTo(endTime) < 0) { // 当前时间更新
				back = true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	static public boolean inRange(Timestamp start_1, Timestamp end_1, Timestamp start_2, Timestamp end_2) {
		...
		{
			// log.fine( "TimeUtil.InRange - No", start_1 + "->" + end_1 + "
			// <??> " + start_2 + "->" + end_2);
			return false;
		}
		// case c
	...
	static public boolean inRange(Timestamp start, Timestamp end, boolean OnMonday, boolean OnTuesday,
		...
		if (calStart.get(Calendar.YEAR) == calEnd.get(Calendar.YEAR)
			...
			if ((!OnSaturday && dayStart == Calendar.SATURDAY) || (!OnSunday && dayStart == Calendar.SUNDAY)
					...
					|| (!OnThursday && dayStart == Calendar.THURSDAY) || (!OnFriday && dayStart == Calendar.FRIDAY)) {
				// log.fine( "TimeUtil.InRange - SameDay - Yes", start + "->" +
				// end + " - "
				// +
		...
		//
		// log.fine( "TimeUtil.inRange - WeekDay Start=" + dayStart + ",
		// Incl.End=" + dayEnd);
	...
	static public boolean isAllDay(Timestamp start, Timestamp end) {
		...
		if (calStart.get(Calendar.HOUR_OF_DAY) == calEnd.get(Calendar.HOUR_OF_DAY)
				...
				&& calStart.get(Calendar.SECOND) == 0 && calStart.get(Calendar.MILLISECOND) == 0 && start.before(end))
			return true;
		//
		return false;
	} // isAllDay
	...
	static public int getDaysBetween(Timestamp start, Timestamp end) {
		...
		// + cal.get(Calendar.DAY_OF_YEAR) + ", dayEnd=" +
	...
	static public Timestamp addDays(Timestamp day, int offset) {
		...
		if (offset == 0)
			return new Timestamp(cal.getTimeInMillis());
		cal.add(Calendar.DAY_OF_YEAR, offset); // may have a problem with
												// negative (before 1/1)
		return new Timestamp(cal.getTimeInMillis());
	} // addDays
	 ...
	 * 
	 * @param start
	 *            start time or null for now
	 * @param end
	 *            end time or null for now
	...
	/**
	 * Returns the day border by combining the date part from dateTime and time
	 * part form timeSlot. If timeSlot is null, then first milli of the day will
	 * be used (if end == false) or last milli of the day (if end == true).
	 * 
	 * @param dateTime
	 * @param timeSlot
	 * @param end
	 ...
	 * 
	 * @param day
	 *            Day; if null current time will be used
	 * @param offset
	 *            months offset
